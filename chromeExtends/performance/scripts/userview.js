;(globalScope => {
		...
		getSelectedIndex([iClientX, iClientY]) {
			...
			while (findIndex <= cacheProfile.rowRectList.length - 1) {
				...
				if (
					...
					iClientY >= rectItem.sy &&
					iClientY <= rectItem.sy + rectItem.h
				) {
					findId = rectItem.id
					break
				}
	...
	const eventManager = () => {
		...
		const documentClickHandler = evte => {
			...
			if (cacheProfile.isShowUserViewPanel) {
				operaManager.togglePanelShow(
					((cacheProfile.isShowUserViewPanel = !cacheProfile.isShowUserViewPanel), cacheProfile.isShowUserViewPanel)
				)
			}
		}
