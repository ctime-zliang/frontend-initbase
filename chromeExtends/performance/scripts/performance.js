;(globalScope => {
	...
	const bindEvent = () => {
		...
		const documentMouseMoveHandler = evte => {
			...
			if (
				evte.clientX >= panelRect.left &&
				evte.clientX <= panelRect.right &&
				evte.clientY >= panelRect.top &&
				evte.clientY <= panelRect.bottom
			) {
		...
		const documentVisiblityChangeHandler = evte => {
			...
			cacheProfile.visiblityChangeTimer = globalScope.setTimeout(
				statusText => (cacheProfile.visibilityState = statusText),
				300,
	...
	const samplingCallbackManager = {
		...
		fillRAFPolylineBlockData(size, polylineRectAreaBottomY) {
			cacheProfile.rAFRatioCycleAverageYPositions = [].concat(
				cacheProfile.rAFRatioCycleAverageYPositions,
				new Array(size).fill(polylineRectAreaBottomY)
			)
		},
		calcRAFPolylineData(polylineRectAreaTop, polylineRectAreaHeight) {
			cacheProfile.rAFRatioCycleAverageYPositions.push(
				polylineRectAreaTop +
					((cacheProfile.maxRAFRatioCycleAverage - cacheProfile.rAFRatioCycleAverage) / cacheProfile.maxRAFRatioCycleAverage) *
						polylineRectAreaHeight
			)
		...
		calcRIdleCommonData() {
			cacheProfile.rIdleRatioCycleAverage =
				cacheProfile.rICIntervalCount / (cacheProfile.maxRAFRatioCycleAverage * (cacheProfile.refreshViewDiffTime / 1000))
		},
		fillRIdlePolylineBlockData(size, polylineRectAreaTop) {
			cacheProfile.rIdleRatioCycleAverageYPositions = [].concat(
				cacheProfile.rIdleRatioCycleAverageYPositions,
				new Array(size).fill(polylineRectAreaTop)
			)
		},
		...
		calcSystemMemoryUsagePolylineData(polylineRectAreaTop, polylineRectAreaHeight) {
			cacheProfile.memoryUsageCycleAverageYPositions.push(
				polylineRectAreaTop + (1 - cacheProfile.memoryUsageCycleAverage) * polylineRectAreaHeight
			)
	...
	const drawManager = {
		...
		drawSystemCpuUsageText(fillStartX, fillStartY) {
			...
			cacheProfile.ctx.fillStyle = operaManager.calcMatchColor(
				viewDataManager.data.cpuUsageCycleAverage,
