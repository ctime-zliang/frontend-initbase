...
export function DoubleTapView(): React.ReactElement {
	...
	useEffect((): (() => void) => {
		if (!controllerRef.current.isInit) {
			...
			controllerRef.current.gestureInstance.addDoubleTapListener(
				(
					...
					if (gestureInteractiveElementRef.current) {
						...
						environment.tapRippleInstance.apply(
							...
							{ x: pageX, y: pageY },
							{
								rippleColor: `rgba(206, 145, 120, 1.0)`,
							}
						)
					}
	...
	return (
		<div className={styles['view-subject']} data-flush={flush}>
			<div className={styles['view-subject-title']}>DoubleTap 事件</div>
