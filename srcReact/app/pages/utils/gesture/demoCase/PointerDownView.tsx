...
export function PointerDownView(): React.ReactElement {
		...
		if (!controllerRef.current.isInit) {
			...
			controllerRef.current.gestureInstance.addPointerDownListener(
				(
					...
					controllerRef.current.downClientX = data.clientX
					controllerRef.current.downClientY = data.clientY
	...
	return (
		<div className={styles['view-subject']} data-flush={flush}>
			<div className={styles['view-subject-title']}>PointerDown 事件</div>
			<div className={styles['view-subject-content']}>
				<div className={styles['gesture-interactive']} ref={gestureInteractiveElementRef}>
					<span>
						触发位置: ({controllerRef.current.downClientX}, {controllerRef.current.downClientY})
					</span>
				</div>
			</div>
		</div>
