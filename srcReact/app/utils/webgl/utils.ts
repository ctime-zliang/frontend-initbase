export function initWebGLContext(canvasElement: HTMLCanvasElement): WebGLRenderingContext | null {
	return canvasElement.getContext('webgl')
}

export function createWebGLShader(gl: WebGLRenderingContext, type: GLenum, source: string): WebGLShader | null {
	...
	const result: any = gl.getShaderParameter(shader, gl.COMPILE_STATUS)
	if (result) {
		return shader
	}
...
export function createWebGLProgram(gl: WebGLRenderingContext, vertexShaderSource: string, fragmentShaderSource: string): WebGLProgram | null {
	const vertexShader: WebGLShader | null = createWebGLShader(gl, gl.VERTEX_SHADER, vertexShaderSource)
	const fragmentShader: WebGLShader | null = createWebGLShader(gl, gl.FRAGMENT_SHADER, fragmentShaderSource)
	...
	const result: any = gl.getProgramParameter(program, gl.LINK_STATUS)
	if (result) {
		return program
	}
...
export function ven$getWebGLVariableLocation(
	...
	cfg: {
		glAttributes: Array<string>
		glUniforms: Array<string>
	} = {
		glAttributes: [],
		glUniforms: [],
	}
	...
	const result: {
		glAttributes: { [key: string]: GLint }
		glUniforms: { [key: string]: WebGLUniformLocation | null }
	} = { glUniforms: {}, glAttributes: {} }
	for (let i: number = 0; i < glAttributes.length; i++) {
		const item: string = glAttributes[i]
		result.glAttributes[item] = gl.getAttribLocation(program, item)
	}
	for (let i: number = 0; i < glUniforms.length; i++) {
		const item: string = glUniforms[i]
 ...
 */
export function initArrayBufferForLaterUse(gl: WebGLRenderingContext, data: ArrayBuffer = new Float32Array([])): WebGLBuffer | null {
...
export function initElementArrayBufferForLaterUse(gl: WebGLRenderingContext, data: ArrayBuffer = new Float32Array([])): WebGLBuffer | null {
	const buffer: WebGLBuffer = gl.createBuffer()
	if (!buffer) {
		console.error('failed to create the buffer object.')
		return null
...
export function loadImageResourceTexture(
	gl: WebGLRenderingContext,
	index: number,
	src: string,
	callback: (gl: WebGLRenderingContext, index: number, texture: WebGLTexture) => void,
	optional: {
		isFlipY?: boolean
	}
) {
