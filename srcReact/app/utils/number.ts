export function onlyPositiveFloatNumber(content: { value: string }, decimalLength: number = 2): { value: string } {
	...
	decimalRegStr += '^(\\-)*(\\d+)\\.('
	for (let i: number = 0; i < decimalLength; i++) {
		decimalRegStr += `\\d`
	}
	decimalRegStr += ').*$'
...
export function onlyRationalFloatNumber(content: { value: string }, decimalLength: number = 2): { value: string } {
	content.value = content.value.replace(/[^\d.-]/g, '')
	content.value = content.value.replace(/^\./g, '')
	content.value = content.value.replace(/\.{2,}/g, '.')
	content.value = content.value.replace('.', '$#$').replace(/\./g, '').replace('$#$', '.')
